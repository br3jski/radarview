...
$manifest = Resolve-Path "releases/v2.2.2/SHA256SUMS-2.2.2"
$signature = Resolve-Path "releases/v2.2.2/SHA256SUMS-2.2.2.sig"
...
if ($LASTEXITCODE -ne 0 -or $version.Trim() -ne "2.2.2") { throw "Windows binary smoke test failed." }
...
try {
    ...
    if ($statusResponse.version -ne "2.2.2" -or [string]::IsNullOrWhiteSpace($statusResponse.installationId)) {
        throw "Windows status page API smoke test failed."
    }
    $statusPage = Invoke-WebRequest -UseBasicParsing -Uri "http://127.0.0.1:54321/"
    if ($statusPage.Content -notmatch "UPDATE AVAILABLE" -or $statusPage.Content -notmatch "FEEDER STATUS" -or $statusPage.Content -match "No feeder token") {
